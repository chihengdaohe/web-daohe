...
trait Event {
	...
	/**
	 * When a post is deleted, we can also remove logs that belong to a specific post type maybe
	 * @return void
	 */
	public static function set_auto_remove_logs() {
		add_action( 'delete_post', static function ( $post_id ) {
			//Logs::instance()->remove_by( 'item_id', $post_id );
		} );
	}
	...
	final public function log() {
		return Logs::instance()->insert( $this );
	}
}
